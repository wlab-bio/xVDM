...
class libObj:
    ...
    def __init__(self, settingsfilename = "lib.settings", output_prefix = "", do_partition_fastq = True, output_subsampling = True):
        ...
        sysOps.throw_status("self.mySettings = " + str(self.mySettings))
...
def subsample(seqform_for_params,seqform_rev_params):
    ...
    #
    # IMPORTANT:
    # Downstream steps (hashAlignments -> line_sorted_clust_uxi* -> assign_umi_pairs)
    # assume the first column (source line/read id) is globally unique across ALL
    # part_* files in a directory. The original library satisfies this because the
    # id comes from a single filtered_src_data stream. During subsampling we must
    # preserve this uniqueness; otherwise duplicate ids across different part files
    # can scramble read-to-UXI alignment after independent sorts, changing what is
    # counted in pairing_stats.txt.
    #
    # To keep part files merge-sortable without an extra sort pass, we renumber
    # the first column to a unique, non-overlapping range per part file (offset by
    # the cumulative counts of earlier part files) *within each subsample depth*.
    sysOps.throw_status('Subsampling ...')

    readcounts = np.loadtxt(sysOps.globaldatapath +  'readcounts.txt', delimiter = ',', dtype=np.int64)
    ...
    # Draw a feasible per-seqform allocation for each subsample depth *before* creating the directory.
    # (The previous implementation created the directory first and could leave empty/partial subdirs.)
    this_sub_readcounts = list()
    valid_subsample_readcounts = list()
    for this_read_count in subsample_readcounts:
        # Sample per-seqform counts without replacement (multivariate hypergeometric).
        # This guarantees draw[i] <= readcounts[i,2] and sum(draw) == this_read_count.
        n_seqforms = int(readcounts.shape[0])
        draw = np.zeros(n_seqforms, dtype=np.int64)
        remaining_draw = int(this_read_count)
        remaining_total = int(tot_retained_reads)
        for i_seqform in range(n_seqforms - 1):
            ngood = int(readcounts[i_seqform,2])
            nbad = int(remaining_total - ngood)
            if remaining_draw <= 0 or ngood <= 0:
                di = 0
            else:
                di = int(np.random.hypergeometric(ngood, nbad, remaining_draw))
            draw[i_seqform] = di
            remaining_draw -= di
            remaining_total -= ngood
        draw[n_seqforms - 1] = remaining_draw
        this_sub_readcount = draw

        # Create directory & settings only for feasible depths
        subdir = sysOps.globaldatapath + 'sub' + str(this_read_count)
        try:
            os.mkdir(subdir)
        except:
            pass

        sysOps.sh("mkdir -p " + subdir + "//")
        sysOps.sh(r"sed 's/\(.*\)\.\.\(.*\)\.\./\1\.\.\/\/\.\.\2\.\./' " + sysOps.globaldatapath + 'lib.settings > ' + subdir + '/tmp_lib.settings') # replace ..// with ..//..//
        sysOps.sh("sed -e '/^-uei_matchfilepath /d' " + subdir + '/tmp_lib.settings > ' + subdir + '/lib.settings') # remove UEI-matching, which will be a poorly-defined problem under sub-sampling
        #os.remove(sysOps.globaldatapath + 'sub' + str(this_read_count) + '/tmp_lib.settings')

        this_sub_readcounts.append(this_sub_readcount.copy())
        valid_subsample_readcounts.append(int(this_read_count))

    subsample_readcounts = valid_subsample_readcounts
    if len(subsample_readcounts) == 0:
        sysOps.throw_status('No subsamples generated (tot_retained_reads=' + str(tot_retained_reads) + ').')
        return

    # First, prepare the randomly sorted files outside the loop
    for i_seqform in range(readcounts.shape[0]):
        for_index = int(readcounts[i_seqform,0])
        rev_index = int(readcounts[i_seqform,1])
        input_file = sysOps.globaldatapath + "part_" + str(for_index) + "_" + str(rev_index) + ".txt"

        if sysOps.check_file_exists("part_" + str(for_index) + "_" + str(rev_index) + ".txt"):
            # Create temp directory if it doesn't exist
            temp_dir = sysOps.globaldatapath + "tmp//"
            sysOps.sh("mkdir -p " + temp_dir)

            # Randomly sort the file and store in temp location
            sorted_file = temp_dir + "sorted_part_" + str(for_index) + "_" + str(rev_index) + ".txt"
            sysOps.sh("awk '{print rand() \"\\t\" $0}' " + input_file +
                    " | sort -T " + temp_dir + " -k1,1 > " + sorted_file)

            for this_sub_readcount,this_read_count in zip(this_sub_readcounts,subsample_readcounts):
                n_lines = int(this_sub_readcount[i_seqform])

                # Destination file
                output_file = sysOps.globaldatapath + "sub" + str(this_read_count) + "//part_" + str(for_index) + "_" + str(rev_index) + ".txt"

                # If this seqform contributes 0 reads at this depth, do not create an empty part-file
                # (empty part-files break stack_uxis(), which assumes any existing part_* can be partitioned).
                if n_lines <= 0:
                    try:
                ...
                # Offset renumbering to keep source ids unique across *all* part files in this subsample
                offset = int(np.sum(this_sub_readcount[:i_seqform]))

                # Extract the required number of lines, remove the random value, and renumber with an offset
                sysOps.sh("head -n " + str(n_lines) + " " + sorted_file +
                        " | cut -f2- > " + sysOps.globaldatapath + "sub" + str(this_read_count) + "//tmp_part_" + str(for_index) + "_" + str(rev_index) + ".txt")

                tmp_output_file = sysOps.globaldatapath + "sub" + str(this_read_count) + "//tmp_part_" + str(for_index) + "_" + str(rev_index) + ".txt"
                sysOps.sh("awk -F, -v off=" + str(offset) + " 'BEGIN{OFS=\",\"} NR==1{len=length($1)} {$1=sprintf(\"%0\"len\"d\",off+NR-1); print}' "
                          + tmp_output_file + " > " + output_file)
                os.remove(tmp_output_file)

            sysOps.sh("rm -f " + sorted_file)
